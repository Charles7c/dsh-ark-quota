...
- **用量节奏告警**：主角口径是「额度进度 vs 时间进度」——时间过了百分之几、额度用了百分之几，即开周期以来的平均节奏（与 AWS Budgets / GCP Billing 同类告警同一口径），重置后立刻可用，也不会被某一天的重度使用带飞。结论直接陈述事实：「时间过了 40%，额度用了 60%，余量偏紧」；撞线时 5 小时 / 周档给出「预计 MM-DD HH:mm 用完，比重置早 N 天」，月档只陈述进度不给具体日期（近期窗口外推一个月没有可信度）。近期消耗速度由窗口内快照做最小二乘回归得出，作为辅助信号放在悬停详情里，并标注「正在加速 / 放缓」与样本数。进度条上的竖线刻度标出平均节奏的投影位置。快照通过 `ctx.storageDomain` 落盘，重启后观测历史不丢。
...
- 节奏结论以「额度进度 ÷ 时间进度」为准（开周期以来的平均节奏，不需要快照历史，重置后立刻可用）；周期刚起步（时间进度不足 10%）时暂不下结论。悬停可见近期消耗速度（5 小时档看近 30 分钟、周档看近 12 小时、月档看近 24 小时，窗口内全部快照做最小二乘回归）以及观测跨度与样本数。
...
> 额度响应里的 `burn` 字段（按账号、按周期）给出用量节奏，两个信号主次分明。**主角是平均节奏**：`paceRatio`（额度进度 ÷ 时间进度，开周期以来的平均燃烧倍率）、`projectedAtReset`（按平均节奏外推到重置时的预计已用百分比，可能 >100）、`timeProgress` / `quotaProgress`（时间进度与额度进度）。**配角是近期速率**：`perDay`（窗口内快照最小二乘斜率，%/天）、`budgetPerDay`（匀速用完刚好到重置的预算速度）、`ratio`（近期倍率 = perDay ÷ budgetPerDay）、`trendRatio`（近期倍率 ÷ 平均倍率，>1 表示正在加速）、`exhaustAt`（按近期速度的预计耗尽时刻，毫秒）、`sampleMs` / `samples`（近期速率的观测跨度与样本数）。`status`（`ok` / `warn` / `over`）以平均节奏投影为准（≥100% 撞线、≥85% 余量紧张）；上游未给重置时刻或周期刚起步时退回近期倍率口径。
