...
/**
 * 由快照序列算出一个周期的消耗速度（业界 burn-rate 口径）。
 *
 * 三个量缺一不可，只报「%/天」是有歧义的：2%/天 对月度周期很安全，
 * 对 5 小时周期是灾难。
 *
 * - `perDay`：燃烧速率，把观测窗口内的 Δ百分比 外推成每天多少百分点；
 * - `budgetPerDay`：预算速率 = 100% / 周期长度，即"匀速用完刚好到重置"；
 * - `ratio`：燃烧倍率 = perDay / budgetPerDay。1.0 刚好够用，>1 会提前耗尽；
 * - `exhaustAt`：按当前速率预计耗尽的时刻（速率为 0 或已满时为 null）。
 *
 * 取样策略：找窗口起点**之前**最近的一条作为基线（否则窗口刚开时样本不足会
 * 算出虚高的速率），窗口内一条都没有就返回 null 而不是猜。
 *
 * 重置对齐（订阅制的关键口径）：`exhaustAt` 假设额度可以无限消耗，但订阅额度
 * 到 `resetAt` 就重置——晚于重置的"耗尽时刻"永远不会发生。所以只要上游给了
 * `resetAtSec`，就另外算两个量：
 * - `projectedAtReset`：按当前速率外推到重置时刻的已用百分比（可能 >100，
 *   ≥100 表示撑不到重置）；
 * - `timeProgress`：本周期时间已经过去的比例（0~1）。与额度进度对比即可知
 *   消耗是超前还是落后于时间。
 * 有投影时状态分档以投影为准（它用的是真实剩余时间与真实当前水位，比倍率
 * 更贴近"这个周期会不会撞线"）；没有 resetAt 时退回倍率口径。
 * @param snaps - 快照序列。
 * @param level - session / weekly / monthly。
 * @param percentUsed - 当前已用百分比。
 * @param nowMs - 当前时刻。
 * @param resetAtSec - 上游返回的重置时刻（unix 秒），可缺省。
 * @returns 速率对象，样本不足时返回 null。
 */
export function burnRate(snaps, level, percentUsed, nowMs, resetAtSec) {
  ...
  // 基线：窗口起点之前最近的一条；没有就退而取序列里最早的一条。
  let base = null;
  for (const s of snaps) {
    if (!Number.isFinite(s?.p?.[level])) continue;
    if (s.t <= from) base = s;
    else if (base === null) { base = s; break; }
  }
  if (base === null) return null;
  const elapsedMs = nowMs - base.t;
  // 观测跨度太短（<2 分钟）时任何抖动都会被放大成离谱的速率。
  if (elapsedMs < 2 * 60 * 1000) return null;
  let delta = percentUsed - base.p[level];
  // 周期重置会让百分比回落（例如 98% → 3%）。这不是"负消耗"，
  // 而是换了一个周期，此时旧基线不可用。
  if (delta < 0) return null;
  const perDay = (delta / elapsedMs) * 86400000;
  const budgetPerDay = (100 / periodMs) * 86400000;
  const ratio = budgetPerDay > 0 ? perDay / budgetPerDay : null;
  const remaining = Math.max(0, 100 - percentUsed);
  const exhaustAt = perDay > 0 && remaining > 0
    ? nowMs + (remaining / perDay) * 86400000
    : null;
  // ── 重置对齐投影 ─────────────────────────────────────────────
  // resetAt 是上游给的真实重置时刻；周期长度用 PERIOD_MS 近似（上游不返回
  // 周期起点）。重置时刻已过（上游尚未刷新）时投影无意义，留 null。
  let projectedAtReset = null;
  let timeProgress = null;
  if (typeof resetAtSec === "number" && Number.isFinite(resetAtSec) && resetAtSec > 0) {
    const remainMs = resetAtSec * 1000 - nowMs;
    if (remainMs > 0 && remainMs <= periodMs + 60 * 60 * 1000) {
      projectedAtReset = percentUsed + (perDay * remainMs) / 86400000;
      timeProgress = Math.min(1, Math.max(0, 1 - remainMs / periodMs));
    }
  }
  // 分档供前端取色：ok（够用）/ warn（偏快）/ over（明显超速）。
  // 有投影时以投影为准：≥100% 必然撞线，≥85% 余量紧张；
  // 没有 resetAt 时退回倍率口径。
  let status;
  if (projectedAtReset !== null) {
    status = projectedAtReset >= 100 - 0.5 ? "over" : projectedAtReset >= 85 ? "warn" : "ok";
  } else {
    status = ratio === null ? null : ratio <= BURN_OK ? "ok" : ratio <= BURN_WARN ? "warn" : "over";
  }
  return {
    level,
    perDay,
...
export function apply(ctx, config) {
  ...
  /** 账号清单（不含任何凭据，只有 id / 标签 / 是否已配密钥 / 关联的 provider）。 */
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      const payload = fresh ? caches.get(account.id).payload : await refresh(account);
