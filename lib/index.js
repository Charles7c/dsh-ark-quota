 ...
 * 均值）；当前水位作为最新观测点参与回归。
 *
 * 跨周期重置（百分比回落，如 98% → 3%）不是负消耗：从最新点往回扫，
 * 遇到回落就把回落点及其之前全部丢弃。跨度不足或净斜率为负时返回 null
 * （不猜）。
...
function recentSlope(snaps, level, percentUsed, nowMs, windowMs, minSpanMs) {
  ...
  // 跨重置截断：从新往旧遇到百分比回落，丢弃回落点及其之前全部。
  for (let i = pts.length - 1; i > 0; i--) {
    if (pts[i - 1].p[level] > pts[i].p[level] + 1e-9) {
      pts.splice(0, i);
      break;
    }
  }
  ...
  // 最小二乘斜率：Σ(t-t̄)(p-p̄) / Σ(t-t̄)²（单位：百分比/毫秒）。
  const n = pts.length;
  let sumT = 0;
  let sumP = 0;
  for (const s of pts) {
  ...
  return { perDay: slopePerMs * 86400000, span, samples: n };
}
 ...
 * - **近期速率（配角）**：窗口内全部快照对当前水位做最小二乘斜率得到
 *   `perDay`，表达「最近烧得多快」；`ratio` 是它相对预算速率的倍率，
 *   `trendRatio` 是近期倍率 ÷ 平均倍率（>1 说明最近在加速）。短期档
 *   （5 小时/周）超速时 `exhaustAt` 给出按近期速度的预计用完时刻；月度档
 *   不展示点时刻——24 小时窗口外推 30 天没有可信度。
 *
 * 状态分档以节奏投影为准（≥100% 撞线 / ≥85% 余量紧张 / 其余正常）；
 * 上游没给 resetAt、或周期刚起步（时间进度 <10%）时退回近期倍率口径。
 * 两个信号都算不出来时返回 null。
 *
 * 只报「%/天」是有歧义的：2%/天 对月度周期很安全，对 5 小时周期是灾难，
 * 所以预算速率与倍率始终一起给出。
 * @param snaps - 快照序列。
...
export function burnRate(snaps, level, percentUsed, nowMs, resetAtSec) {
  ...
  const exhaustAt = perDay !== null && perDay > 0 && remaining > 0
    ? nowMs + (remaining / perDay) * 86400000
    : null;

  // ── 节奏投影（主角）：额度进度 ÷ 时间进度 ─────────────────────
  // resetAt 是上游给的真实重置时刻；周期长度用 PERIOD_MS 近似（上游不返回
  // 周期起点）。重置时刻已过（上游尚未刷新）或远得离谱时投影无意义。
  ...
  // 有投影时以投影为准；没有 resetAt 或周期刚起步时退回近期倍率口径。
  let status = null;
