	...
	factory: (require) => {
		...
		function burnTooltip(b, now, resetAtSec) {
			...
			if (sliding) {
				...
				// 第二行：时间/额度进度 + 平均节奏投影结论。
				if (typeof b.timeProgress === "number" && typeof b.quotaProgress === "number") {
					var line = "时间 " + Math.round(b.timeProgress * 100)
						+ "% · 额度 " + Math.round(b.quotaProgress * 100) + "%";
					if (typeof b.projectedAtReset === "number" && isFinite(b.projectedAtReset)) {
						line += b.projectedAtReset >= 100
							? "，平均节奏会用完"
							: "，平均节奏重置时用到 " + Math.round(b.projectedAtReset) + "%";
					}
					lines.push(line);
				}
				// 第三行：近期速度的用完时刻（比平均口径更早）。
				if (typeof b.exhaustAt === "number" && isFinite(b.exhaustAt) && b.exhaustAt > now) {
		...
		function QuotaRow(_a) {
			...
			if (burn) {
				if (item.level === "session") {
					if (typeof burn.perDay === "number" && isFinite(burn.perDay) && burn.perDay > 0) {
						var aging = (typeof burn.agingPerDay === "number" && isFinite(burn.agingPerDay))
							? burn.agingPerDay : burn.budgetPerDay;
						if (aging > 0) {
							fcEnd = Math.min(100, burn.perDay / aging * 100);
							fcTitle = "按近期速度，水位将升至约 " + Math.round(fcEnd) + "% 后趋稳";
						}
					}
				} else {
					var remainMs = item.resetAt ? item.resetAt * 1000 - now : 0;
					var recentEnd = null;
					if (remainMs > 0 && typeof burn.perDay === "number" && isFinite(burn.perDay) && burn.perDay > 0) {
						recentEnd = usedPct + burn.perDay * remainMs / 86400000;
					}
					var avgEnd = typeof burn.projectedAtReset === "number" && isFinite(burn.projectedAtReset)
						? burn.projectedAtReset : 0;
					fcEnd = Math.max(recentEnd || 0, avgEnd);
					if (recentEnd !== null && recentEnd >= avgEnd) {
						fcTitle = "按近期速度，重置时将用到约 " + Math.round(Math.min(100, recentEnd)) + "%";
					} else if (avgEnd > 0) {
						fcTitle = "按平均节奏，重置时预计用到 " + Math.round(avgEnd) + "%";
					}
				}
			}
