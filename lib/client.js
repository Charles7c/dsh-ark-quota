window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function applyDisplayMode(next) {
			var v = next === "used" ? "used" : "remaining";
		...
		function QuotaRow(_a) {
			...
			return jsxs("div", {
				...
				children: [
					...
					jsx("div", {
						style: { width: "100%", minWidth: 0, height: "6px", borderRadius: "3px", background: "var(--dsw-alias-track-bg, rgba(128,128,128,0.25))", overflow: "hidden" },
						children: jsx("div", {
		...
		function Card(_a) {
			var state = _a.state, onRefresh = _a.onRefresh, loading = _a.loading, dm = _a.displayMode;
			var now = useNow();
			var data = state.data;
			var cardOuter = {
			...
			return jsxs("div", {
				...
				children: [
					...
					data && fetched ? jsxs("div", {
						...
						children: [
							jsxs("span", { title: "上次更新：" + fmtClockMs(fetched), children: [
								fmtRelativeMs(fetched, now),
								" · ",
		...
		function ArkQuotaWidget(_a) {
			...
			return jsx(Card, { state: { data: q.data, error: q.error }, loading: q.loading, displayMode: dm, onRefresh: function () { reload(true); } });
		}
