	...
	factory: (require) => {
		...
		function burnColor(status) {
			return Object.prototype.hasOwnProperty.call(BURN_COLORS, status)
				? BURN_COLORS[status]
				: "var(--dsh-alias-label-tertiary)";
		}

		// 一条额度行的用量节奏提示（悬浮时给全量口径，行内只放一个短标签）。
		function burnTooltip(b) {
			if (!b) return "";
			var lines = [];
			// 近期速率（配角）：窗口内快照回归出的斜率，标注观测跨度与样本数，
			// 让用户知道这个速度是从多少数据算出来的。
			if (typeof b.perDay === "number" && isFinite(b.perDay)) {
				var obs = "近期消耗 " + fmtBurn(b.perDay) + "（观测 " + fmtSpan(b.sampleMs);
				if (typeof b.samples === "number") obs += " · " + b.samples + " 个样本";
				lines.push(obs + "）");
			}
			lines.push("预算速度 " + fmtBurn(b.budgetPerDay) + "（匀速用完刚好到重置）");
			if (typeof b.ratio === "number" && isFinite(b.ratio)) {
				lines.push("近期倍率 " + (Math.round(b.ratio * 100) / 100) + "×");
			}
			// 节奏投影（主角）：额度进度 vs 时间进度——开周期以来的平均口径，
			// 不靠快照历史，也不会被某一天的重度使用带飞。
			if (typeof b.projectedAtReset === "number" && isFinite(b.projectedAtReset)) {
				...
				if (tp !== null && qp !== null) {
					...
					lines.push("时间进度 " + tp + "% · 额度进度 " + qp + "%"
						+ (pace >= 5 ? "（消耗超前于时间）" : pace <= -5 ? "（消耗落后于时间）" : "（与时间进度持平）"));
				}
				if (b.projectedAtReset >= 100) {
					lines.push("按周期内平均节奏，重置前就会用完（重置时将达 " + Math.round(b.projectedAtReset) + "%）");
				} else {
					lines.push("按平均节奏，重置时预计用到 " + Math.round(b.projectedAtReset) + "%");
				}
				// 近期相对平均节奏在加速还是放缓。
				if (typeof b.trendRatio === "number" && isFinite(b.trendRatio)) {
					if (b.trendRatio >= 1.5) lines.push("近期消耗明显快于平均节奏，正在加速");
					else if (b.trendRatio <= 0.6) lines.push("近期消耗慢于平均节奏，正在放缓");
				}
			} else if (typeof b.exhaustAt === "number" && isFinite(b.exhaustAt)) {
				lines.push("按近期速度预计 " + fmtClockMs(b.exhaustAt) + " 用完");
			}
			return lines.join("\n");
		}
		...
		// 用量节奏的一句话结论（面板主角信息）。主角口径是「额度进度 vs
		// 时间进度」——开周期以来的平均节奏，不靠快照历史，重置后立刻可用，
		// 也不会被某一天的重度使用带飞。三档语义：
		//   over：平均节奏下重置前就会用完 → 短期档（5 小时/周）给出预计
		//         用完时刻（还来得及切账号）；月度档只陈述进度，不给具体
		//         日期——近期窗口外推一个月没有可信度；
		//   warn：重置时预计用到 85%~100% → 余量偏紧；
		//   ok：<85% → 节奏正常。
		// 上游未给 resetAt 时退回近期速率的线性耗尽时刻；都算不出时返回 null。
		function burnHeadline(burn, item, now) {
			if (!burn) return null;
			var projected = burn.projectedAtReset;
			if (typeof projected === "number" && isFinite(projected)) {
				var tp = typeof burn.timeProgress === "number" ? Math.round(burn.timeProgress * 100) : null;
				var qp = typeof burn.quotaProgress === "number" ? Math.round(burn.quotaProgress * 100) : null;
				var paceText = (tp !== null && qp !== null)
					? "时间过了 " + tp + "%，额度用了 " + qp + "%"
					: "";
				if (projected >= 100 - 0.5) {
					// 撞线：短期档给预计用完时刻；月度档不给点估计，陈述进度即可。
					var shortTier = item.level === "session" || item.level === "weekly";
					if (shortTier && typeof burn.exhaustAt === "number" && isFinite(burn.exhaustAt) && burn.exhaustAt > now) {
						var text = "预计 " + fmtClockMs(burn.exhaustAt) + " 用完";
						if (item.resetAt) {
					...
					return {
						text: paceText + (paceText ? "，" : "") + "按此节奏重置前会用完",
						status: "over"
					};
				}
				var tense = projected >= 85 ? "warn" : "ok";
				return {
					text: paceText + (paceText ? "，" : "") + (tense === "warn" ? "余量偏紧" : "节奏正常"),
					status: tense
				};
			}
			// 没有重置投影（上游未给 resetAt）：退回近期速率的线性耗尽时刻。
		...
		function QuotaRow(_a) {
			...
			// 节奏结论行（主角信息）：危险（warn/over）时任何一档都醒目显示；
			// 安全时只在月度行给一句"节奏正常"，5 小时/周档安全时不啰嗦。
			// 主角口径是额度进度 vs 时间进度，不需要快照历史——重置后立刻有
			// 结论；只有周期刚起步（时间进度 <10%）或上游不给重置时刻时才
			// 没有结论行，那时不留占位，避免用"积累中"暗示一个不存在的等待。
			var headline = burnHeadline(burn, item, now);
			var showHeadline = headline !== null
				&& (headline.status !== "ok" || item.level === "monthly");
			// 三行布局：
			//   上行「分类 · 近期消耗速度 …… 百分比 · 重置倒计时」
			//   中行整宽进度条（含平均节奏投影刻度）
			//   下行用量节奏结论（危险时醒目，月度安全时也给一句）
			return jsxs("div", {
				title: tip,
				...
				children: [
					jsxs("div", {
						...
						children: [
							...
							burn && typeof burn.perDay === "number" && isFinite(burn.perDay) ? jsx("span", {
					...
					// 进度条：填充为已用百分比；竖线刻度为「按周期内平均节奏
					// 外推到重置时」的位置（超过 100% 贴右端）。
					jsxs("div", {
							...
							jsxs("div", {
								...
								children: [
									...
									burn && typeof burn.projectedAtReset === "number" && isFinite(burn.projectedAtReset)
										...
										? jsx("div", {
											title: "按周期内平均节奏，重置时预计用到 " + Math.round(burn.projectedAtReset) + "%",
											style: {
					...
					// 节奏结论：warn/over 档醒目着色，月度安全档用弱化色。
		...
		function RailPill(_a) {
			...
			var title = (pct === null ? "方舟额度（无数据）" : "方舟额度 · " + label + fmtPct(pct) + "%")
				+ (hasError ? " · " + state.error : "")
				+ (hasError ? " · 点击打开设置" : " · 点击展开侧边栏");
			return jsx("button", {
				type: "button",
				title: title,
				"aria-label": hasError ? "方舟额度出错，点击打开设置" : "方舟额度，点击展开侧边栏",
				onClick: function () {
				...
				style: {
					...
					color: hasError ? "var(--dsw-alias-state-error-primary, #e5484d)" : "var(--dsw-alias-label-primary)",
