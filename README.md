...
- **Pace alerts**: the headline signal is quota progress vs time progress — the average burn since the period started (the same basis AWS Budgets / GCP Billing alerts use). It needs no snapshot history, works immediately after a reset, and cannot be thrown off by one heavy-usage day. The verdict states facts: "40% of the period elapsed, 60% of quota used — running tight"; when on track to hit the wall, the 5-hour and weekly tiers give "projected to run out MM-DD HH:mm, N days before reset", while the monthly tier states progress without a date (extrapolating a 24 h window over a month is not trustworthy). Recent burn is derived from an ordinary-least-squares fit over in-window snapshots, shown as a supporting signal in the hover detail (with sample count and "speeding up / cooling down" trend). A tick on the progress bar marks the average-pace projection. Snapshots are persisted through `ctx.storageDomain`, so the observation history survives a restart.
...
- With two or more accounts a compact dropdown appears in the card header. Switching swaps the quota figures and pace verdict, and writes the choice back as the new default.
- The verdict is based on quota progress ÷ time progress (the average pace since the period started; no snapshot history needed, available right after a reset); it stays silent while less than 10% of the period has elapsed. The hover detail shows recent burn (a 30-min window for the 5-hour tier, 12 h for weekly, 24 h for monthly — an OLS fit over all in-window snapshots) plus the observation span and sample count.
...
> The `burn` field on a quota payload (per account, per tier) carries two signals, one primary and one supporting. **Average pace (primary):** `paceRatio` (quota progress ÷ time progress — the average burn multiplier since the period started), `projectedAtReset` (projected used % at reset at the average pace, may exceed 100), `timeProgress` / `quotaProgress`. **Recent burn (supporting):** `perDay` (OLS slope over in-window snapshots, in %/day), `budgetPerDay` (the even pace that would exactly hit 100% at reset), `ratio` (recent burn ÷ budget), `trendRatio` (recent ratio ÷ average ratio; >1 means speeding up), `exhaustAt` (projected run-out time at the recent rate, ms), `sampleMs` / `samples` (observation span and sample count for the recent rate). `status` (`ok` / `warn` / `over`) follows the average-pace projection (≥100% hits the wall, ≥85% tight); when the upstream gives no reset time or the period just started, it falls back to the recent-ratio basis.
